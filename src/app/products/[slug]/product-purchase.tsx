...
export function ProductPurchase({
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h2 className="text-sm font-semibold text-zinc-300 mb-3 flex items-center gap-2"><Calendar className="h-4 w-4 text-[#f5c451]" /> {t(L, 'buy.periods')}</h2>
        {multi ? (
                ...
                >
                  <div>
                    <div className="font-semibold text-white flex items-center gap-2 flex-wrap">
                      ...
                      {v.name}
                      {isBest && <span className="text-[10px] uppercase tracking-wide bg-[#f5c451] text-black font-bold rounded-full px-2 py-0.5">{t(L, 'buy.bestValue')}</span>}
                    </div>
                    <div className="text-xs text-zinc-500 mt-0.5 pl-6">
                      {v.duration_days ? `${v.duration_days} ${t(L, 'buy.days')}` : ''}
                      {v.perDay !== null && <span className="text-zinc-600"> • {v.perDay.toFixed(2)} {currency}{t(L, 'buy.perDay')}</span>}
                    </div>
                  </div>
        ...
        {multi && (
          <p className="text-[11px] text-zinc-600 mt-2">
            {selected ? t(L, 'buy.selectedHint') : t(L, 'buy.pickHint')}
          </p>
        )}
      </div>
      ...
      <Card className="border-[#22d3ee]/25 bg-[#22d3ee]/[0.04]">
        <CardContent className="p-5">
          <h3 className="font-semibold text-white flex items-center gap-2 mb-1"><MessageCircle className="h-4 w-4 text-[#22d3ee]" /> {t(L, 'buy.howTo')}</h3>
          <p className="text-sm text-zinc-400 mb-4">
            {multi ? (selected ? t(L, 'buy.howToSelected') : t(L, 'buy.howToPick')) : t(L, 'buy.howToSingle')}
          </p>
          <ContactButtons channels={channels} productName={productName} selected={selected ? variants.find(v => v.id === selected) || null : null} currency={currency} lang={L} />
          {channels.length === 0 && <p className="text-sm text-amber-400">{t(L, 'buy.channelsSoon')}</p>}
        </CardContent>
      </Card>
    </div>
  )
